...
class Tetris:
    ...
    def reset(self):
        ...
        self.score = 0          # Score
        self.tetrominoes = 0    # Pieces
        self.cleared_lines = 0  # Cleared Lines
        self.bag = list(range(len(self.pieces)))  # 用块的种类数构建顺序序列
        random.shuffle(self.bag)   # 打乱序列
        self.ind = self.bag.pop()  # 随机拿出一个序号
        self.piece = [row[:] for row in self.pieces[self.ind]]  # 按序号取得方块
        self.current_pos = {"x": self.width // 2 - len(self.piece[0]) // 2, "y": 0}  # 取当前位置
        self.gameover = False  # 游戏未结束
    ...
    def get_state_properties(self, board):
        lines_cleared, board = self.check_cleared_rows(board)  # 检查Cleared Lines并删除
        holes = self.get_holes(board)  # 统计空洞数量
